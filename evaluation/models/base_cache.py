...
class BaseCompressCache(DynamicCache):
    ...
    def __init__(self, compression_ratio: float, enable_monitor: bool = True, **kwargs):
        super().__init__()
    ...
    def update(self, key_states: torch.Tensor, value_states: torch.Tensor, layer_idx: int, cache_kwargs=None) -> tuple[
        ...
        # 3. 完美还原你指定的监控格式 (仅在第 0 层打印)
        if self.enable_monitor and layer_idx == 0:
            print(f"[KV Monitor] Stage: {stage_name:<7} | "
